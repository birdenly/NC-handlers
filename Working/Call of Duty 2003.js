...
Game.Play = function () {
  ...
  var players = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + '\\main'
  if (!System.IO.Directory.Exists(players)) {
      System.IO.Directory.CreateDirectory(players);
  }
  

  if (!System.IO.File.Exists(Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\main\\config_mp.cfg")){
    
    
    if (System.IO.File.Exists(Context.RootInstallFolder + "\\main\\config_mp.cfg")){

      var savePath = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\main\\config_mp.cfg";
      var savePkgOrigin = System.IO.Path.Combine(Context.RootInstallFolder, "main\\config_mp.cfg");
      System.IO.File.Copy(savePkgOrigin, savePath, true);

    }else{
      var savePath = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\main\\config_mp.cfg";
      var savePkgOrigin = System.IO.Path.Combine(Game.Folder, "config_mp.cfg");
      System.IO.File.Copy(savePkgOrigin, savePath, true);
  ...
  if (System.IO.File.Exists(Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\CoD1MeatBots_RC2\\config_mp.cfg")) {

    var cfgpath = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\CoD1MeatBots_RC2\\config_mp.cfg";
    var dict = [
  ...
  var cfgpath = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\main\\config_mp.cfg";
  var dict = [
    Context.FindLineNumberInTextFile(cfgpath, 'seta r_mode', Nucleus.SearchType.StartsWith) + '|seta r_mode "-1"',
    Context.FindLineNumberInTextFile(cfgpath, 'seta com_maxfps', Nucleus.SearchType.StartsWith) + '|seta com_maxfps "' + FPS + '"',
    Context.FindLineNumberInTextFile(cfgpath, 'seta r_customheight', Nucleus.SearchType.StartsWith) + '|seta r_customheight "' + Context.Height + '"',
  ...
  if (!Context.IsKeyboardPlayer) {
    var savePath =  Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\main\\Controller.cfg";
  ...
  if(BOTS=="yes"){
    ...
    var Path = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\CoD1MeatBots_RC2";
    System.IO.Directory.CreateDirectory(Path);

    Context.CopyFolder(Context.ScriptFolder + "\\CoD1MeatBots_RC2", Context.GetFolder(Nucleus.Folder.InstancedGameFolder)+ "\\CoD1MeatBots_RC2");

    var cfgpath = Context.GetFolder(Nucleus.Folder.InstancedGameFolder) + "\\CoD1MeatBots_RC2\\server_listen.cfg";
    var dict = [
      Context.FindLineNumberInTextFile(cfgpath, 'set cod_mbot_amount1', Nucleus.SearchType.StartsWith) + '|set cod_mbot_amount1 "' + AMOUNT + '"',
      Context.FindLineNumberInTextFile(cfgpath, 'set cod_mbot_amount2', Nucleus.SearchType.StartsWith) + '|set cod_mbot_amount2 "' + AMOUNT + '"',
      Context.FindLineNumberInTextFile(cfgpath, 'set cod_pbot_amount1', Nucleus.SearchType.StartsWith) + '|set cod_pbot_amount1 "' + AMOUNT + '"',
      Context.FindLineNumberInTextFile(cfgpath, 'set cod_pbot_amount2', Nucleus.SearchType.StartsWith) + '|set cod_pbot_amount2 "' + AMOUNT + '"',
    ...
    start += " +set fs_game CoD1MeatBots_RC2 +exec server_listen.cfg +map_rotate"
    }else{
      start += " +set fs_game CoD1MeatBots_RC2"
    }
