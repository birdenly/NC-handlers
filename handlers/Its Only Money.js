...
Game.DirSymlinkExclusions = ["Engine\\Binaries\\ThirdParty\\Steamworks\\Steamv153\\Win64", "CrimeRP\\Binaries\\Win64"];
Game.HandlerInterval = 100;
Game.SymlinkExe = false;
Game.SymlinkGame = true;
Game.SymlinkFolders = true;
Game.KeepSymLinkOnExit = false;
Game.ExecutableName = "CrimeRP-Win64-Shipping.exe";
Game.BinariesFolder = "CrimeRP\\Binaries\\Win64";
Game.SteamID = "1415220";
Game.GUID = "Its Only Money";
Game.GameName = "Its Only Money";
Game.MaxPlayers = 4;
Game.Hook.ForceFocus = false;
Game.Hook.ForceFocusWindowName = "It's Only Money";
Game.Hook.DInputEnabled = false;
Game.Hook.XInputEnabled = false;
Game.Hook.XInputReroute = false;
Game.Hook.CustomDllEnabled = false;
Game.XInputPlusDll = [];
Game.Description = "IMPORTANT: Start the game once and change some settings before trying split screen. THIS GAME IS EXTREMELY HEAVY TO RUN AND HAS MANY CRASHES EVEN WITHOUT NUCLEUS.\n\nPlay the tutorial until you unlock the multiplayer option in your phone, than just invite the other players\n\nIf you use keyboards and mice after all the instances have launched, resized and positioned correctly, press the END key once to lock the input for all instances to have their own working cursor and keyboard. You need to left click each mouse to make the emulated cursors appear after locking the input. Press the END key again to unlock the input when you finish playing. You can also use CTRL+Q to close Nucleus and all its instances when the input is unlocked.";
...
Game.UserProfileConfigPath = "AppData\\Local\\CrimeRP\\Saved\\Config";
Game.UserProfileSavePath = "AppData\\Local\\CrimeRP\\Saved\\SaveGames";
...
Game.Play = function() {
  ...
  var Args = (Context.Args = " -d3d11 -windowed " + " -AlwaysFocus " + " -ResX= " + Context.Width + " -ResY= " + Context.Height);
  ...
  var savePath = Context.EnvironmentPlayer + Context.UserProfileConfigPath + "\\Windows\\Settings.ini";
  Context.ModifySaveFile(savePath, savePath, Nucleus.SaveType.INI, [
  ...
  if (Context.PlayerID == 0) {
    Context.ModifySaveFile(savePath, savePath, Nucleus.SaveType.INI, [new Nucleus.IniSaveInfo("/Script/Engine.GameUserSettings", "MusicAudioVolume", "0.700000")]);
  } else {
    Context.ModifySaveFile(savePath, savePath, Nucleus.SaveType.INI, [new Nucleus.IniSaveInfo("/Script/Engine.GameUserSettings", "MusicAudioVolume", "0.000000")]);
  }
  ...
  var lines = [
    "2139120=It's Only Money: Founders Pack Upgrade"
  ];
